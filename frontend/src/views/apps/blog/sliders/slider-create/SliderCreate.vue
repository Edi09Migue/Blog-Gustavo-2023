...
import useSliderCreate from './useSliderCreate'
...
export default {
  ...
  setup() {
    ...
    const resetSliderData = () => {
      sliderData.value = JSON.parse(JSON.stringify(modeloPagina))
      errorServer.value = null
    }

    
    const PAGINAS_APP_STORE_MODULE_NAME = "blog-sliders";
      // Register module
      if (!store.hasModule(PAGINAS_APP_STORE_MODULE_NAME))
          store.registerModule(PAGINAS_APP_STORE_MODULE_NAME, slidersStoreModule);
      // UnRegister on leave
      onUnmounted(() => {
        if (store.hasModule(PAGINAS_APP_STORE_MODULE_NAME))
    ...
    const onSubmit = () => {
        store.dispatch("blog-sliders/addSlider", sliderData.value).then((response) => {
