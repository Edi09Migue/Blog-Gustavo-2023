                ...
                >
                    ...
                    >
                        ...
                        >
                            <b-form-input
                                id="guard_name"
                                v-model="inscritoData.guard_name"
                                :state="getValidationState(validationContext)"
                                trim
                            />
                    ...
                    >
                        ...
                        >
                            <b-form-input
                                id="group_key"
                                v-model="inscritoData.group_key"
                                :state="getValidationState(validationContext)"
                                trim
                            />
...
export default {
    ...
    methods: {
        validateRemoteField(field, value) {
            return store
                .dispatch("app-inscrito/validateUnique", {
                    field: field,
                    value: value
                })
                .then(response => {
                    return {
                        valid: this.inscritoData.id
    ...
    setup(props, { emit }) {
        ...
        const inscritoData = ref(
            JSON.parse(JSON.stringify(props.inscrito))
        );

        const errorServer = ref(props.errores);

        const resetInscritoData = () => {
            inscritoData.value = JSON.parse(JSON.stringify(props.inscrito));
            originalName.value = inscritoData.value.name;
            errorServer.value = null;
        };

        var propiedades = toRefs(props);
        watch(propiedades.inscrito, () => {
            resetInscritoData();
        });
        ...
        const onSubmit = () => {
            ...
            if (inscritoData.value.id)
                emit("update-inscrito", inscritoData.value, closeSidebar);
            else emit("add-inscrito", inscritoData.value, closeSidebar);
        };
        ...
        const {
            ...
            resetForm,
            clearForm
        } = formValidation(resetInscritoData, props.clearInscritoData);
