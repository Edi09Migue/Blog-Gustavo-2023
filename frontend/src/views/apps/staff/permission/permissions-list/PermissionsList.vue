...
export default {
    ...
    setup() {
        ...
        const permissionData = ref(
            JSON.parse(JSON.stringify(blankPermissionData))
        );

        const resetPermissionData = () => {
            permissionData.value = JSON.parse(
                JSON.stringify(blankPermissionData)
            );
        };

        const addPermission = val => {
            store
                .dispatch("app-permission/addPermission", val)
                .then(response => {
        ...
        const removePermission = function(id) {
            this.$swal({
                ...
                if (result.value) {
                    store
                        .dispatch("app-permission/removePermission", id)
                        .then(() => {
                            this.$swal({
                                icon: "success",
        ...
        const updatePermission = (val, callback) => {
            store
                .dispatch("app-permission/updatePermission", val)
                .then(response => {
                    if (response.data.status) {
                        toast({
                            component: ToastificationContent,
