...
export default {
  ...
  methods:{
    validateRemoteField(field, value){
      return store.dispatch('app-permission/validateUnique',{ field: field, value: value }).then((response) => {
        return {
        valid: this.permissionData.id ? (this.originalName != value ? response.data.valid : true) : response.data.valid,
        data: {
  ...
  setup(props, { emit }) {
    ...
    const onSubmit = () => {
        ...
        if (permissionData.value.id) emit('update-permission', permissionData.value)
        else emit('add-permission', permissionData.value)
